...
def load_sql(cursor, sqlfile):
    '''
    Runs a sequence of SQL statements

    @param cursor: postgres cursor
    @param sqlfile: pathanme to a sql file
    @return: updated cursor
    '''
    with open(sqlfile) as sql_file:
        sql_stmts = sql_file.read()
        run_sql = sqlparse.split(sql_stmts)
...
def get_check_constraint_code(cursor, table):
    ''' Check constraints belong to table attributes'''
    stmt = "select oid from pg_class where relname='{}';".format(table)
    cursor.execute(stmt)
    ...
    if result:
        oid = result[0][0]
        print(oid)
        stmt = "select check_clause from information_schema.check_constraints \
                where constraint_name like '%_{}_%';".format(oid)
        cursor.execute(stmt)
        result = cursor.fetchall()
        print(result)
        if result:
    ...
    def get_test(self):
        ...
        try:
            ...
            if connection:
                ...
                stmt = 'drop database {};'.format(student)
                try:
                    cursor.execute(stmt)
